...
const ordemColuna = { // true = crescente, false = descrescente
    'nome': true,
    'sobrenome': true,
    'ddd': true,
    'número': true
...
function aoClicarNoTitulo( ev ) {
    ...
    const comparar = ( c1, c2 ) => {
        switch ( coluna ) {
            case 'nome'     : return compararValores( c1.nome, c2.nome, ordemColuna[ coluna ] );
            case 'sobrenome': return compararValores( c1.sobrenome, c2.sobrenome, ordemColuna[ coluna ] );
            case 'ddd'      : return compararValores( c1.tel.ddd, c2.tel.ddd, ordemColuna[ coluna ] );
            case 'número'   : return compararValores( c1.tel.numero, c2.tel.numero, ordemColuna[ coluna ] );
        }
    };

    const resultado = contatos.sort( comparar );
    desenharContatos( resultado );
    // Inverte a ordenaçao para usar no próximo clique
    ordemColuna[ coluna ] = ! ordemColuna[ coluna ];
}


// function pesquisaFiltro( pesquisa ) {
//     return e => e.nome.includes( pesquisa ) ||
...
function pesquisar( ev ) {
    ...
    const resultado = contatos.filter( e => e.nome.includes( pesquisa ) ||
        e.sobrenome.includes( pesquisa ) ||
        e.tel.ddd.includes( pesquisa ) ||
        e.tel.numero.includes( pesquisa) );

    // const resultado = contatos.filter( pesquisaFiltro( pesquisa ) );
